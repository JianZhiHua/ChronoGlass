...
**ChronoGlass** 是一个基于 `Python + PyQt6` 的 Windows 桌面悬浮时钟与提醒工具，集成 `系统时钟`、`搬砖`、`倒计时`、`秒表`、`闹钟` 五种模式。它强调低干扰、常驻桌面、快速切换和本地持久化，适合长期停放在桌面侧边使用。

整体界面采用小清新浅色主题，主窗口无边框、半透明、始终置顶，支持托盘常驻和任意位置拖动。

## 功能概览

- 五种模式一体化：系统时钟、搬砖、倒计时、秒表、闹钟管理
...
- GIF 会在页面中直接播放
- 全局右键菜单可打开搬砖设置，修改一级标题、完成后标题、目标时间、进行中图片和完成图片
- 倒计时进行中不显示完成后标题；目标时间已到后，页面会显示 `恭喜`、完成后标题和完成态图片
...
| 左键单击托盘图标 | 显示或隐藏主窗口 |
| 右键单击托盘图标 | 打开托盘菜单 |
| 右键主窗口 | 打开全局右键菜单，可切换模式、重置计时、打开搬砖设置或闹钟设置 |
| 点击右上角 `⇆` | 按顺序切换 5 种模式 |
| 点击右上角 `—` | 隐藏到托盘 |
| 点击右上角 `×` | 彻底退出程序 |
